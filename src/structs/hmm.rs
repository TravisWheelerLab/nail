...
impl Hmm {
    ...
    pub fn from_p7hmm_file<R: AsRef<Path> + Display>(path: R) -> Result<Vec<Self>> {
        let phmm_file = File::open(&path)?;
        let mut phmm_lines = BufReader::new(phmm_file).lines();

        let mut p7hmm_structs: Vec<Hmm> = vec![Hmm::new()];
        ...
        let mut current_p7hmm = p7hmm_structs.get_mut(0).unwrap();
                ...
                ParserState::Header => {
                    ...
                    match flag {
                        ...
                        P7_HEADER_REFERENCE_FLAG => {
                            let value = get_token_as_str(&tokens, 1).with_context(error_context)?;
                            current_p7hmm.header.has_reference_annotation = match value {
                                "yes" => true,
                                "no" => false,
                        ...
                        P7_HEADER_MASK_FLAG => {
                            let value = get_token_as_str(&tokens, 1).with_context(error_context)?;
                            current_p7hmm.header.has_model_mask = match value {
                                "yes" => true,
                                "no" => false,
                        ...
                        P7_HEADER_CONSENSUS_RESIDUE_FLAG => {
                            let value = get_token_as_str(&tokens, 1).with_context(error_context)?;
                            current_p7hmm.header.has_consensus_residue = match value {
                                "yes" => true,
                                "no" => false,
                        ...
                        P7_HEADER_CONSENSUS_STRUCTURE_FLAG => {
                            let value = get_token_as_str(&tokens, 1).with_context(error_context)?;
                            current_p7hmm.header.has_consensus_structure = match value {
                                "yes" => true,
                                "no" => false,
                        ...
                        P7_HEADER_MAP_FLAG => {
                            let value = get_token_as_str(&tokens, 1).with_context(error_context)?;
                            current_p7hmm.header.has_map_annotation = match value {
                                "yes" => true,
                                "no" => false,
                        ...
                        P7_HEADER_COMMAND_FLAG => {
                            ...
                            current_p7hmm.header.command_line_history =
                                get_joined_tokens(&tokens, 1).with_context(error_context)?;
                        }
                        P7_HEADER_NSEQ_FLAG => {
                            current_p7hmm.header.num_sequences =
                                get_token_as_usize(&tokens, 1).with_context(error_context)?;
                        }
                        P7_HEADER_EFFN_FLAG => {
                            current_p7hmm.header.effective_num_sequences =
                                get_token_as_f32(&tokens, 1).with_context(error_context)?;
                        }
                        P7_HEADER_CHECKSUM_FLAG => {
                            current_p7hmm.header.checksum =
                                get_token_as_usize(&tokens, 1).with_context(error_context)?;
                        }
                        P7_HEADER_GATHERING_FLAG => {
                            current_p7hmm.header.gathering_thresholds = [
                                get_token_as_f32(&tokens, 1).with_context(error_context)?,
                                get_token_as_f32(&tokens, 2).with_context(error_context)?,
                            ]
                        }
                        P7_HEADER_TRUSTED_FLAG => {
                            current_p7hmm.header.trusted_cutoffs = [
                                get_token_as_f32(&tokens, 1).with_context(error_context)?,
                                get_token_as_f32(&tokens, 2).with_context(error_context)?,
                            ]
                        }
                        P7_HEADER_NOISE_FLAG => {
                            current_p7hmm.header.noise_cutoffs = [
                                get_token_as_f32(&tokens, 1).with_context(error_context)?,
                                get_token_as_f32(&tokens, 2).with_context(error_context)?,
                            ]
                        }
                        P7_HEADER_STATS_FLAG => {
                            let mu_or_tau =
                                get_token_as_f32(&tokens, 3).with_context(error_context)?;
                            let lambda =
                                get_token_as_f32(&tokens, 4).with_context(error_context)?;
                            let sub_flag =
                                get_token_as_str(&tokens, 1).with_context(error_context)?;
                            match sub_flag {
                                "LOCAL" => {
                                    let sub_sub_flag =
                                    ...
                                    match sub_sub_flag {
                                        P7_HEADER_STATS_MSV_FLAG => {
                                            current_p7hmm.stats.msv_gumble_mu = mu_or_tau;
                                            current_p7hmm.stats.msv_gumble_lambda = lambda;
                                        }
                                        P7_HEADER_STATS_VITERBI_FLAG => {
                                            current_p7hmm.stats.viterbi_gumble_mu = mu_or_tau;
                                            current_p7hmm.stats.viterbi_gumble_lambda = lambda;
                                        }
                                        P7_HEADER_STATS_FORWARD_FLAG => {
                                            current_p7hmm.stats.forward_tau = mu_or_tau;
                                            current_p7hmm.stats.forward_lambda = lambda;
                                        }
                ...
                ParserState::ModelHead => {
                    ...
                    match body_parser_state {
                        ModelParserState::MatchEmissions => match flag {
                            P7_BODY_COMPO_FLAG => {
                                current_p7hmm.model.composition =
                        ...
                        ModelParserState::InsertEmissions => {
                            current_p7hmm.model.insert_probabilities[0] =
                        ...
                        ModelParserState::StateTransitions => {
                            current_p7hmm.model.transition_probabilities[0] =
                ...
                ParserState::ModelBody => {
                    ...
                    match flag {
                        P7_BODY_END_FLAG => {
                            ...
                            assert_eq!(
                                current_p7hmm.model.match_probabilities.len(),
                                current_p7hmm.header.model_length + 1 as usize
                            );
                            assert_eq!(
                                current_p7hmm.model.insert_probabilities.len(),
                                current_p7hmm.header.model_length + 1 as usize
                            );

                            p7hmm_structs.push(Hmm::new());
                            current_p7hmm = p7hmm_structs.last_mut().unwrap();
                        ...
                        _ => match body_parser_state {
                            ModelParserState::MatchEmissions => {
                                ...
                                if flag == line_number_flag {
                                    current_p7hmm.model.match_probabilities.push(
                                        get_tokens_as_probability_vec(&tokens, 1, 21)
                                            .with_context(error_context)?,
                                    );
                            ...
                            ModelParserState::InsertEmissions => {
                                current_p7hmm.model.insert_probabilities.push(
                                    get_tokens_as_probability_vec(&tokens, 0, 20)
                                        .with_context(error_context)?,
                                );
                                body_parser_state = ModelParserState::StateTransitions;
                            ...
                            ModelParserState::StateTransitions => {
                                current_p7hmm.model.transition_probabilities.push(
                                    get_tokens_as_probability_vec(&tokens, 0, 7)
                                        .with_context(error_context)?,
                                );
                                current_model_line_number += 1;
...
impl fmt::Debug for Hmm {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        writeln!(f, "model length: {}", self.header.model_length)?;

        for model_position in 0..self.header.model_length {
            writeln!(f, "{}", model_position)?;
