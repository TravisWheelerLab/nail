...
pub fn forward(profile: &Profile, target: &Sequence, dp_matrix: &mut impl DpMatrix) -> Result<()> {
    ...
    dp_matrix.set_special(0, Profile::SPECIAL_N, 0.0);
    ...
    for target_idx in 1..=target.length {
        ...
        dp_matrix.set_special(target_idx, Profile::SPECIAL_E, -f32::INFINITY);
        ...
        dp_matrix.set_match(
            ...
            log_sum!(
                dp_matrix.get_match(target_idx - 1, profile.length - 1)
                    + profile.transition_score(Profile::PROFILE_MATCH_TO_MATCH, profile.length - 1),
                dp_matrix.get_insert(target_idx - 1, profile.length - 1)
                    + profile
                        .transition_score(Profile::PROFILE_INSERT_TO_MATCH, profile.length - 1),
                dp_matrix.get_special(target_idx - 1, Profile::SPECIAL_B)
                    + profile.transition_score(Profile::PROFILE_BEGIN_TO_MATCH, profile.length - 1),
                dp_matrix.get_delete(target_idx - 1, profile.length - 1)
                    + profile
                        .transition_score(Profile::PROFILE_DELETE_TO_MATCH, profile.length - 1)
        ...
        dp_matrix.set_delete(
            ...
            log_sum!(
                dp_matrix.get_match(target_idx, profile.length - 1)
                    + profile
                        .transition_score(Profile::PROFILE_MATCH_TO_DELETE, profile.length - 1),
                dp_matrix.get_delete(target_idx, profile.length - 1)
                    + profile
                        .transition_score(Profile::PROFILE_DELETE_TO_DELETE, profile.length - 1)
            ),
        );
