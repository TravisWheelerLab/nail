...
use crate::io::{FastaParser, P7HmmParser, ReadSeekExt, ReadState, SeekableTake};
    ...
    R: RecordParser,
{
    ...
    pub fn iter(&'_ self) -> DatabaseIter<'_, R, impl DoubleEndedIterator<Item = &str> + '_> {
        DatabaseIter {
            inner: self.clone(),
            keys: self.index.keys().map(|k| k.as_str()),
        }
    }
